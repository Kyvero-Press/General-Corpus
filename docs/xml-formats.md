...
For `.tex`, `.latex`, and `.pdf` output, `pandoc-cme-xml` defaults to Junicode (`Junicode-Regular.otf` from TeX Live's `junicode` package) and automatically adds the project LaTeX pagination header and Lua filter in `scripts/pandoc-latex-pagebreaks.*` to reduce orphaned/widowed prose and verse fragments. Pass your own Pandoc `mainfont` variable to override the default font.
